...
template<class T> constexpr T qmi(T a, ll b, ll m) { T res{ 1 }; while (b) {  if (b & 1) res = res * a % m; cout << a << " "; a = a * a % m; b >>= 1; }return res; }
...
void solve(){
    ...
    if(s[0] == '?' && s[n - 1] == '?'){
        int ans = 0;
        ans = ans + 2 * (qmi(2, cnt_wen - 2, mod)) % mod * (n - 2) % mod;
        ans = ans % mod;
        // ans = ans * 2;
        // ans = ans % mod;
        ans = ans + 2 * (qmi(2, cnt_wen - 2, mod)) % mod * 2 % mod;   
        cout << ans % mod << endl;
    }
    else if(s[0] == '?' && s[n - 1] != '?'){
        int ans = 0;
        ans += (qmi(2, cnt_wen - 1, mod)) % mod * (n - 2) % mod;
        ans = ans % mod;
        ans = ans + (qmi(2, cnt_wen - 1, mod)) % mod * 2 % mod;
        cout << ans % mod << endl;
    }
    else if(s[0] != '?' && s[n - 1] == '?'){
        int ans = 0;
        ...
        int ans = 0;
        ans += (qmi(2, cnt_wen, mod)) % mod * (n - 2) % mod;
        ans = ans % mod;
        cout << ans % mod << endl;
    }
    else{
        int ans = 0;
        ans += (qmi(2, cnt_wen, mod)) % mod * 2 % mod;
        cout << ans % mod << endl;
    }
...
signed main(){
    // cout << qmi(2, 2e5, mod) << endl;
    cout << qmi((ll)2, 200000, 1000000007) << endl;
