...
void solve(){
    ...
    //求 next 数组 
    for(int i = 2, j = 0; i <= m; i ++){   //i从2开始的原因是因为 next[1] 一定为0
        while(j && b[i] != b[j + 1]) j = ne[j];   //当 j 不等于 0 时，即不匹配且可以后退的时候回退
        if(b[i] == b[j + 1]) j ++;
        ne[i] = j;   // 说明数组 b[i - j + 1: i] 与 b[1 : j] 相等，即 b[i] 可以回退到 b[j];
    }
