...
void init(){
    a1[1] = 1, a1[2] = 0;
    for(int i = 3; i < N; i ++){
        a1[i] = a1[i - 1] % mod + a1[i - 2] % mod;
        a1[i] = a1[i] % mod;
    }
    for(int i = 1;i < N; i ++){
        s1[i] = s1[i - 1] % mod + a1[i] % mod;
        s1[i] = s1[i] % mod;
    }

    a2[1] = 0, a2[2] = 1;
    for(int i = 3; i < N; i ++){
        a2[i] = a2[i - 1] % mod + a2[i - 2] % mod;
    }
    for(int i = 1;i < N; i ++){
        s2[i] = s2[i - 1] % mod + a2[i] % mod;
    }
}
...
void solve(){
    ...
    for(int i = 1; i <= n; i ++){
        ans = ans + a[i] % mod * s1[n - i + 1] % mod;
        ans = ans % mod;
        ans = ans + a[i] % mod * s2[i] % mod;
        ans = ans % mod;
    }
    cout << ans << endl;
