...
const ll N = 2e6 + 5, inf = 2e9, M = 105;
const ld eps = 1e-12;
...
void solve(){
    ...
    ll ans = 1;
    ans = (ans * c(n - n / 3, x)) % mod;
    ans = (ans * c(n / 3, x)) % mod;
    ans = (ans * jiecheng(n / 3)) % mod;
    ans = (ans * jiecheng(n - n / 3)) % mod;

    // ll ans = c(n - n / 3, x) % mod *  c(n / 3, x) % mod * jiecheng(n / 3) % mod * jiecheng(n - n / 3) % mod;
    cout << ans << endl; 
}
