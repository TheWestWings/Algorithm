...
void solve(){
    ...
    vector<int> ans; 
    for(int i = 1; i < n; i ++){
        if(seg[i].first.first <= seg[i - 1].first.second && col[i - 1] == 0){
            ans.push_back(seg[i].second + 1);
            col[i] = 1;
        }
        if(i >= 2 && seg[i].first.first <= seg[i - 2].first.second){
            // ans.push_back({seg[i].first, seg[i - 2].second});
            cout << -1 << endl;
            return;
        }
    }
