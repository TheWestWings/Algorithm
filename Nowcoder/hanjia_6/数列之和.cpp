...
void solve(){
    ...
    //     //     cout << s[i] - s[i - 1] << endl;
    //     // }
    //     if((i + 1 - 2) * 2 + 10 - s[i] != last){
    //         t.push_back({i + 1, (i + 1 - 2) * 2 + 10 - s[i]});
    //         last = (i + 1 - 2) * 2 + 10 - s[i];
    //     }
    //     cout << i + 1 << " "  << s[i] << " " << toBinary(s[i]) << " " << check(i + 1) << " " << checkReverse(check(i + 1)) << " " << checkReverse(check(i + 1)) - (i + 1) << endl;
    //     int k = i + 1;
    //     cout << (1 << (check(k) + 1)) - 2 - 2 *(checkReverse(check(k)) - (k)) << endl;
    // }

    // for(auto x: t){
    //     cout << x.first << " " << -x.second << " " << check(x.first - 1) << endl;
    // }

    int k = 0;
    cin >> k;
    int t = check(k);
    cout << (1ll << (t + 1)) - 2 - 2ll *(checkReverse(t) - (k)) << endl;
}
