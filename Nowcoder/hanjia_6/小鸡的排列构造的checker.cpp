...
void update(int& node, int pre, int l, int r, int pos) {
    node = ++cnt;
    tree[node] = tree[pre];
    tree[node].sum++;
    if (l == r) return;
    int mid = (l + r) >> 1;
    if (pos <= mid)
        update(tree[node].l, tree[pre].l, l, mid, pos);
    else
        update(tree[node].r, tree[pre].r, mid + 1, r, pos);
}

int query(int u, int v, int l, int r, int x) {
    if (r <= x) return tree[v].sum - tree[u].sum;
    int mid = (l + r) >> 1;
    if (x <= mid)
        return query(tree[u].l, tree[v].l, l, mid, x);
    else
        return tree[tree[v].l].sum - tree[tree[u].l].sum + query(tree[u].r, tree[v].r, mid + 1, r, x);
}
...
void solve(){
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        update(root[i], root[i - 1], 1, n, rank);
    }

    while (m --) {
        int l, r, x;
        cin >> l >> r >> x;
        int rank = getRank(a[x]);
        int cnt = query(root[l - 1], root[r], 1, n, rank);
        cout << cnt + l - 1 << "\n";
    }
}
